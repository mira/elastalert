...
from mira.protocols.http import HttpConnectionError
...
class MiraApi(Api):
	...
	def put(self, path, args=None):
		...
		return super(MiraApi, self).put(path, args)

	def delete(self, path, args=None):
		args = args or {}
		args["api_key"] = self.key
		return super(MiraApi, self).delete(path, args)

	def post(self, path, args=None):
		args = args or {}
		args["api_key"] = self.key
		return super(MiraApi, self).post(path, args)

	def get(self, path, args=None):
		args = args or {}
		args["api_key"] = self.key
		return super(MiraApi, self).get(path, args)
...
class ConfigRegistryApi(MiraApi):
...
class FlightsApi(MiraApi):
	...
	def create_neustar_match(self, advertising_id, neustar_id):
		return self.post(
			os.path.join("neustar", advertising_id),
